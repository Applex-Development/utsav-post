...
function Post({ postId, origuser, username, userId,txt, imageUrl, noLikes }) {
    ...
    useEffect(() => {
        let unsubscribe;
        if (postId) {
            unsubscribe = db.collection("posts").doc(postId).collection("comments").orderBy("ts", "desc").onSnapshot((snapshot) => {
    ...
    return (
        <div className="post">
            ...
            <h4 className="post__text">{txt}</h4>
